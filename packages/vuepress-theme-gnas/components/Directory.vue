<template>
  <div class="directory-wrap">
    ...
    <div class="directory-content-wrap">
      <ul>
        <li
          :key="`directory-h3-${item.value}-${index}`"
          class="directory-h3-li"
          v-for="(item, index) in directory"
          :class="item.type"
        >
          <a @click="scrollTo(item.href)" :title="item.value">{{
            item.value
          }}</a>
        </li>
      </ul>
    ...
    getDirectory() {
      this.directory = [
        ...document.querySelectorAll(
          this.$page.frontmatter.config && this.$page.frontmatter.config.dirTag
            ? this.$page.frontmatter.config.dirTag
            : "h3, h4, h5"
        ),
      ].map((item) => {
